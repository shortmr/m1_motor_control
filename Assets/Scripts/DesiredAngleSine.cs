...
public class DesiredAngleSine : MonoBehaviour
{
    ...
    public void DrawFeedback(LineRenderer renderer) {
        ...
        angleFrequency = settings.GetComponent<ControlSettings>().normalizedAngleFrequency;
        ...
        if (sine == 1) {
            ...
            renderer.positionCount = (int) ((float) pointCount*trialDuration*angleFrequency);

            for (int i = 0; i < renderer.positionCount; i++)
            {
                float x = screenWidth * ((float) i / (float) renderer.positionCount - 0.5f);
                float sin = 0f;
                sin = Mathf.Sin(2f * Mathf.PI * angleFrequency * trialDuration * (float) i / (float) renderer.positionCount);
                float y;
                // dorsiflexion + plantarflexion
                y = romRangeCombined * sin + romOffsetCombined;
                renderer.SetPosition(i, new Vector3(x, gain * y + 2f, 0f));
