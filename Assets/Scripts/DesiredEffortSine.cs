...
public class DesiredEffortSine : MonoBehaviour
{
    ...
    public void DrawFeedback(LineRenderer renderer) {
        ...
        effortFrequency = settings.GetComponent<ControlSettings>().normalizedEffortFrequency;
        ...
        if (sine == 1) {
            ...
            renderer.positionCount = (int) ((float) pointCount*trialDuration*effortFrequency);

            for (int i = 0; i < renderer.positionCount; i++)
            {
                float x = screenWidth * ((float) i / (float) renderer.positionCount - 0.5f);
                float sin = 0f;
                sin = Mathf.Sin(2f * Mathf.PI * effortFrequency * trialDuration * (float) i / (float) renderer.positionCount);
                float y;
                // dorsiflexion + plantarflexion
                y = mvcRangeCombined * sin + mvcOffsetCombined;
                renderer.SetPosition(i, new Vector3(x, gain * y + 2f, 0f));
